...
class TestDetectAutomationGaps:
    ...
    async def test_detects_recurring_pattern(self, behavioral_client, mock_ha_client):
        ...
        for day_offset in range(5):
            dt = (now - timedelta(days=day_offset)).replace(hour=22, minute=0, second=0)
            entries.append(
                {
    ...
    async def test_gap_confidence_increases_with_frequency(self, behavioral_client, mock_ha_client):
        ...
        for day_offset in range(10):
            dt = (now - timedelta(days=day_offset)).replace(hour=7, minute=30, second=0)
            entries.append(
                {
        ...
        gaps = await behavioral_client.detect_automation_gaps(hours=240, min_occurrences=3)
