"""Tests for MCP client DB config resolution and reset.

Tests:
- reset_mcp_client() clears the singleton
- MCP client falls back to env vars when no DB config
- MCP client resolution logic
"""
...
class TestResetMCPClient:
    """Tests for reset_mcp_client()."""

    def test_reset_clears_singleton(self):
        """reset_mcp_client sets the internal _client to None."""
    ...
    def test_get_mcp_client_after_reset_creates_new(self, monkeypatch):
        """After reset, get_mcp_client() creates a new instance."""
        from src.mcp import client as client_mod
        ...
        monkeypatch.setattr(
            "src.mcp.base.get_settings", lambda: settings
        )
        monkeypatch.setattr(
            "src.mcp.base._try_get_db_config", lambda s: None
        )
        ...
        # Get a new one
        new_client = client_mod.get_mcp_client()
...
class TestTryGetDBConfig:
    ...
    def test_returns_none_when_db_raises(self):
        ...
        from src.mcp.base import _try_get_db_config
...
class TestResolveConfig:
    """Tests for BaseMCPClient._resolve_config."""
    ...
    def test_fallback_to_env_vars(self, monkeypatch):
        ...
        monkeypatch.setattr("src.mcp.base.get_settings", lambda: settings)
        monkeypatch.setattr("src.mcp.base._try_get_db_config", lambda s: None)

        config = BaseMCPClient._resolve_config()
    ...
    def test_uses_db_config_when_available(self, monkeypatch):
        ...
        monkeypatch.setattr("src.mcp.base.get_settings", lambda: settings)
        monkeypatch.setattr(
            "src.mcp.base._try_get_db_config",
