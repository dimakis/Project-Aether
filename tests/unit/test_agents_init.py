...
class TestBaseAgentTraceSpan:
    ...
    async def test_trace_span_yields_metadata(self):
        agent = ConcreteAgent(role=AgentRole.ARCHITECT)
        with patch("src.agents.base.emit_progress"):
    ...
    async def test_trace_span_emits_progress(self):
        agent = ConcreteAgent(role=AgentRole.ARCHITECT)
        with patch("src.agents.base.emit_progress") as mock_emit:
    ...
    async def test_trace_span_handles_error(self):
        agent = ConcreteAgent(role=AgentRole.ARCHITECT)
        with (
            patch("src.agents.base.emit_progress"),
            patch("src.agents.base.add_span_event"),
        ):
    ...
    async def test_trace_span_with_state_context(self):
        ...
        with (
            patch("src.agents.base.emit_progress"),
            patch("src.agents.base.get_active_span", return_value=None),
            patch("src.agents.base.add_span_event"),
            patch("src.agents.base.log_param"),
        ):
    ...
    async def test_trace_span_with_mlflow_available(self):
        ...
        with (
            patch("src.agents.base.emit_progress"),
            patch("src.agents.base.get_active_span", return_value=MagicMock()),
            patch("src.agents.base.add_span_event"),
            patch.dict(
                "sys.modules",
                {"mlflow": mock_mlflow},
...
class TestBaseAgentStateContext:
    ...
    def test_log_state_context_none(self):
        agent = ConcreteAgent(role=AgentRole.ARCHITECT)
        with patch("src.agents.base.log_param") as mock_log:
