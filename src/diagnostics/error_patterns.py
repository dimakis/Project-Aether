...
KNOWN_ERROR_PATTERNS: list[_Pattern] = [
    ...
    _Pattern(
        regex=re.compile(
            r"(?:unable to connect|connection (?:timed out|refused|lost|error|reset)|timeout|timed?\s*out)",
            re.IGNORECASE,
        ),
        category="connection",
    ...
    _Pattern(
        regex=re.compile(
            r"(?:auth(?:entication|orization)\s+failed|invalid\s+credentials|access\s+denied|unauthorized|401)",
            re.IGNORECASE,
        ),
    ...
    _Pattern(
        regex=re.compile(
            r"(?:device\s+.*?(?:is\s+)?unavailable|unavailable\s+(?:device|entity|sensor)|not\s+responding)",
            re.IGNORECASE,
        ),
    ...
    _Pattern(
        regex=re.compile(
            r"(?:invalid\s+config|schema\s+validation|expected\s+\w+\s+for|configuration\s+error|yaml\s+error|invalid\s+(?:entry|value|type))",
            re.IGNORECASE,
        ),
    ...
    _Pattern(
        regex=re.compile(
            r"(?:error\s+setting\s+up|setup\s+(?:failed|error)|ConfigEntryNotReady|failed\s+to\s+(?:set\s*up|initialize|load))",
            re.IGNORECASE,
        ),
    ...
    _Pattern(
        regex=re.compile(
            r"(?:database|recorder|sqlite|disk\s+I/O|journal\s+mode|corrupt|migration\s+failed)",
            re.IGNORECASE,
        ),
        category="database",
