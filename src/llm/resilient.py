...
class ResilientLLM(BaseChatModel):
    ...
    def bind_tools(self, tools: Any, **kwargs: Any) -> Any:
        """Delegate bind_tools to the primary LLM.

        BaseChatModel.bind_tools raises NotImplementedError; the actual
        implementation lives on the concrete LLM (e.g. ChatOpenAI).
        __getattr__ cannot intercept this because the method exists on
        the parent class, so we override explicitly.
        """
        return self.primary_llm.bind_tools(tools, **kwargs)

    def __getattr__(self, name: str) -> Any:
        """Delegate attributes not on this wrapper to primary LLM."""
